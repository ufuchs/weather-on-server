...
(function (undefined) {
    ...
    /**
     * paritial @copyright Vincent Schoettke
     *
    ...
    function extractWeather(weatherAsJson, location, cb) {
        ...
        for (i = 0; i < 4; i += 1) {
            ...
            /*
            if (i === 0) {
                // for today we use the sky icon which changes frequently
                weather.forecastday[i].ic = iconMap[forecastday[i].skyicon];
            }
            */
    ...
    wunderground.getWeather = function (params, cb) {
        ...
        fetchIt(params.location, function (err, json) {

            if (err) {
                throw new Error(err);
            }

            extractWeather(json, params.location, function (err, weather) {
