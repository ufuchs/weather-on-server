...
solar.calcTrueLonAndRadiusVec = function (T) {
    ...
    //
    //
    // @see : MEEUS, Astronomical Algorithms (Second Edition), (25.4)
    ...
    var L0 = calcMeanLon(T),
        ...
        e = calcEccentricity(T),
        v = M + C;

    return {
        trueLon : L0 + C,
        radVec : (1.000001018 * (1 - e * e)) / (1 + e * cos(v * base.DEG2RAD))
    };
...
//
//
solar.calcAppRaAndDeclSync = function(jd, nutation) {

    if (!base.isNum(jd)) {
        throw new Error("processSingleDaySync: missing param 'jd'.");
    }

    // if (!base.isNum(ε)) {
    //     throw new Error("processSingleDaySync: missing param 'ε'.");
    // }

    var T = julian.j2000Century(jd, 0),     // Universal Time expected!
        tlrv = solar.calcTrueLonAndRadiusVec(T),
        appPos = solar.calcAppPosition(T, tlrv.trueLon, nutation.ε);
