...
function getSequenceOfJulianDays(jd, numOfDays) {
    ...
    var jd0 = julian.jdAtMidnight(jd),
        i = -2,         // for daylenght we need minus two extra days
        sequence = [];
...
SunCalc.getTimes = function (date, lat, lon) {
    ...
    Q.when(getSequenceOfJulianDays(jd, seqLength))  // <-- returns 0h UT
        .then(function (seq) {
            ...
            var delta = appPos.map(function (ap) { return ap.delta; });
            return {
            ...
            for (begin = 0; begin < seqLength + 1; begin += 1) {
                ...
                tzOff = new Date(date[0], date[1] - 1, date[2], 4, 0, 0).getTimezoneOffset();
...
//            console.log(days);
            ...
            return days.map(function (day) {
                var res = interpol.extremum(day.delta);
                console.log(res);
                return rise.process(c, day);
            });
        }).then(function(rst) {

            rst.map(function (p) {
                console.log(p.date);
                console.log(julian.dec2hhmmssA(p.rise));
                console.log(julian.dec2hhmmssA(p.transit));
                console.log(julian.dec2hhmmssA(p.set));
                console.log();
            });
