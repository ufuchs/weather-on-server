...
function getSequenceOfJulianDays(jd, numOfDays) {
    ...
    var jd0 = julian.jdAtMidnight(jd),
        i = -2,         // for interpolation we need minus one extra day
        sequence = [];
...
////////////////////////////////////////////////////////////////////////////////
    ...
    Q.when(getSequenceOfJulianDays(jd, seqLength))
        .then(function (seq) {
        ...
        .then(function(s) {
            ...
            for (i = 0; i < seqLength + 1; i += 1) {

                var w = s.jd.slice(i, 3 + i);

                var x = s.alpha.slice(i, 3 + i);
                var y = s.delta.slice(i, 3 + i);
                var z = {
                    jd : w,
                    jdm : julian.jdToCalendarGregorian(w[1]),
                    alpha : x,
                    delta : y
                }
                console.log(z);
