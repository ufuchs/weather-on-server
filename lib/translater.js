...
(function (undefined) {
    ...
    // @param {lang} String
    // @return String
    function cssFileName(device, lang, cb) {
        ...
        fs.readdir(deviceCssPath, function (err, files) {
            ...
            if (result.cssFile === null) {
                result.cssFile = device + '.css';
            }
    ...
    Translater.prototype = {
        ...
        translate : function (weather, isoLocale, device, cb) {

            var locale = this.localesMap[isoLocale.toLowerCase()];

            i18n.setLocale(locale);
            moment().lang(locale);

            cssFileName(device, locale, function () {
