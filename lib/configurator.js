...
(function () {
    ...
    function configureSvgFilenames(cfg) {
        ...
        cfg.singleDayDisplay = appCfg.getSingleDayDisplay(device);
        cfg.crushed = appCfg.getCrushed(device);

        // SVG template
        cfg.svg.template = appCfg.getSvgTemplate(device);
        ...
        fs.exists(path.join(deviceDir, cssFile_LangSpecific), function (exists) {
            ...
            if (exists) {
                cfg.svg.cssFile = cssFile_LangSpecific;
            } else {
                cfg.svg.cssFile = cssFile;
            }
    ...
    function configurePngFilenames(cfg) {
        ...
        cfg.png.targetDir = targetDir;
//        cfg.png.basename = path.join(targetDir, appCfg.getPngFilename());
        cfg.png.basename = appCfg.getPngFilename();
    ...
    function makeTargetDir(cfg) {
        ...
        return utils.mkdirW(cfg.png.targetDir)
            .then(function () {
                return cfg;
            });
    ...
    function loadSvgTemplate(cfg) {
        ...
        return utils.readTextFileW(cfg.svg.template)
