...
(function () {
    ...
    svgEngine = {
        ...
        populateSvgTemplate : function (wfo, dayNum) {
            ...
            if (maxForecastDays > fc.length) {
                maxForecastDays = fc.length;
            }

            for (i = 0; i < maxForecastDays; i += 1) {

                replaceSet['dow' + i] = fc[i].dayName;
                replaceSet['h' + i] = fc[i].temp.high;
                replaceSet['l' + i] = fc[i].temp.low;
                replaceSet['ic' + i] = fc[i].ic;
        ...
        localize : function (wfo) {
            ...
            for (i = 0; i < w.maxForecasts; i += 1) {
                arr.push(Q.when(this.createForecast(p, i, singleDayDisplay)));
            }
