...
(function () {
    ...
    function writeResults(wfo) {
        ...
        // render the SVG
        function renderSvgFile2PngFile(file) {

            // reduce the size of the PNG file
            function crushPng(f) {

                var d1 = when.defer(),
                    // `args` rausziehen
                    args = utils.fillTemplates(vendors.cmd.pngcrush, {
                        inPng: f.inPng,
                        outPng: f.outPng
                    });
            ...
            return fn.call(svg2png, file.weatherSvg, file.unweatherPng)
                .then(function (unweatherPng) {
                    return {inPng : unweatherPng, outPng : file.weatherPng};
                })
                .then(crushPng);
        ...
        fs.writeFile(wfo.filenames.out.weatherSvg, wfo.svg, function (err) {
            ...
            if (err) {
                ...
                d.resolve(renderSvgFile2PngFile(wfo.filenames.out));
            }
